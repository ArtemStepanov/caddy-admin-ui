...
describe("Dashboard read-only routes", () => {
  ...
  it("shows drift warning, hides read-only mutations, and displays JSON details", async () => {
      ...
      vi.fn((url: string) => {
        ...
        if (url === "/api/routes/1/details")
          return json({
            ...
            raw_caddy_route: { handle: [{ handler: "custom" }] },
          });
        return json({});
      }),
    );
    ...
    expect(screen.getByText(/guarded by Caddy ETags/)).toBeInTheDocument();
    expect(
      screen.getByText("Unsupported / managed outside UI"),
    ).toBeInTheDocument();
