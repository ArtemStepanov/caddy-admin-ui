...
function lastPostBody(calls: { url: string; init?: RequestInit }[]): any {
  const post = [...calls].reverse().find((c) => c.url === '/api/routes' && c.init?.method === 'POST');
...
describe('RouteForm kind isolation', () => {
  ...
  it('submits reverse_proxy config with strip_path_prefix and route-level headers', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string, init?: RequestInit) => {
      calls.push({ url, init });
      if (url === '/api/routes' && init?.method === 'POST') return json({ route: { id: '1', domain: 'proxy.example.com', handler_type: 'reverse_proxy', config: {} } });
      return json({});
    }));
    ...
    fireEvent.input(screen.getByPlaceholderText('example.com'), { target: { value: 'proxy.example.com' } });
    fireEvent.input(screen.getByPlaceholderText('/*'), { target: { value: '/api/*' } });
    fireEvent.input(screen.getByPlaceholderText('/api'), { target: { value: '/api' } });
    fireEvent.input(screen.getByPlaceholderText('localhost:8080'), { target: { value: 'localhost:8080' } });
  ...
  it('switching to file_server drops strip_path_prefix and proxy-only config', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string, init?: RequestInit) => {
      calls.push({ url, init });
      if (url === '/api/routes' && init?.method === 'POST') return json({ route: { id: '2', domain: 'files.example.com', handler_type: 'file_server', config: {} } });
      return json({});
    }));
    ...
    fireEvent.input(screen.getByPlaceholderText('example.com'), { target: { value: 'files.example.com' } });
    fireEvent.input(screen.getByPlaceholderText('/*'), { target: { value: '/api/*' } });
    fireEvent.input(screen.getByPlaceholderText('/api'), { target: { value: '/api' } });
    fireEvent.input(screen.getByPlaceholderText('localhost:8080'), { target: { value: 'localhost:8080' } });
    fireEvent.click(screen.getAllByText('Add')[0]);
    ...
    fireEvent.input(screen.getByPlaceholderText('/var/www/html'), { target: { value: '/var/www' } });
  ...
  it('switching to redir drops strip_path_prefix and proxy-only config', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string, init?: RequestInit) => {
      calls.push({ url, init });
      if (url === '/api/routes' && init?.method === 'POST') return json({ route: { id: '3', domain: 'redir.example.com', handler_type: 'redir', config: {} } });
      return json({});
    }));
    ...
    fireEvent.input(screen.getByPlaceholderText('example.com'), { target: { value: 'redir.example.com' } });
    fireEvent.input(screen.getByPlaceholderText('/*'), { target: { value: '/api/*' } });
    fireEvent.input(screen.getByPlaceholderText('/api'), { target: { value: '/api' } });
    fireEvent.input(screen.getByPlaceholderText('localhost:8080'), { target: { value: 'localhost:8080' } });
    fireEvent.click(screen.getAllByText('Add')[0]);
    ...
    fireEvent.input(screen.getByPlaceholderText('https://example.com{uri}'), { target: { value: 'https://target.example.com{uri}' } });
  ...
  it('renders a preserved read-only route outside the editable form', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url === '/api/routes/r1') return json({
        route: {
          id: 'r1', domain: 'raw.example.com', handler_type: 'unknown', config: {},
          enabled: true, readonly: true, support_status: 'unsupported_readonly',
          readonly_reason: 'unknown handler', created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z',
        },
      });
      return json({});
    }));
