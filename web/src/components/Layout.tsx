...
export function Layout({ children }: LayoutProps) {
  ...
  const status =
    checking && !statusData ? "loading" : statusData?.status || "offline";
  ...
  return (
    <div class="min-h-screen flex flex-col">
        ...
        <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <nav class="flex gap-4">
              ...
              <a href="/settings" class="text-slate-300 hover:text-white">
                Settings
              </a>
            </nav>
          </div>
      ...
      {status !== "offline" && statusData && !statusData.setup_complete && (
        <div class="bg-amber-900/40 border-b border-amber-700 px-4 py-2 text-center text-sm text-amber-200">
          Caddy is reachable, but route ownership is not configured.{" "}
          <a href="/settings" class="underline">
            Review setup
          </a>
        </div>
