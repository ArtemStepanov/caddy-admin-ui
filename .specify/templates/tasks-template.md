...
**Tests**: The examples below include test tasks. Tests are REQUIRED for sync-critical, security, data-preservation, import/export, storage, and authentication changes; otherwise include tests only if requested in the feature specification.
...
- **Frontend**: `web/src/`, `web/tests/` or colocated `*.test.tsx`
- **Go tests**: colocated `*_test.go` files under `cmd/` and `internal/`
...
- [ ] T010 Add raw Caddy config preservation path if the feature touches import/export/sync
- [ ] T011 Add validation/authentication guardrails if the feature touches admin UI/API access
...
### Tests for User Story 1 (REQUIRED if sync-critical/security/data-preservation, otherwise optional) ⚠️
...
### Tests for User Story 2 (REQUIRED if sync-critical/security/data-preservation, otherwise optional) ⚠️
...
### Tests for User Story 3 (REQUIRED if sync-critical/security/data-preservation, otherwise optional) ⚠️
...
- [ ] TXXX [P] Additional unit tests for sync-critical or security paths
- [ ] TXXX Verify Caddy sync warnings and recovery messaging
