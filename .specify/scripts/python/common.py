...
def resolve_template(name: str, repo_root: Path) -> Path | None:
    """Return the first matching template in Spec Kit's override order."""
    candidates = [repo_root / ".specify" / "templates" / "overrides" / f"{name}.md"]
    presets = repo_root / ".specify" / "presets"
    if presets.is_dir():
        candidates.extend(sorted(presets.glob(f"*/templates/{name}.md")))
    extensions = repo_root / ".specify" / "extensions"
    if extensions.is_dir():
        candidates.extend(sorted(extensions.glob(f"*/templates/{name}.md")))
    candidates.append(repo_root / ".specify" / "templates" / f"{name}.md")
